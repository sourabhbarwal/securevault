...
exports.createSecret = async (req, res) => {
  ...
  try {
    ...
    emitToUser(req.userId, 'vault:created', {
      secretId: secret._id,
      name: secret.name,
    ...
    return ApiResponse.success(res, {
      secret: { _id: secret._id, name: secret.name, category: secret.category },
    }, 'Secret stored.', 201);
  } catch (err) {
    return ApiResponse.error(res, err.message);
  }
...
exports.updateSecret = async (req, res) => {
  ...
  try {
    ...
    emitToUser(req.userId, 'vault:updated', {
      secretId: secret._id,
      isFavorite: secret.isFavorite,
    });
    await auditService.log(req.userId, 'SECRET_UPDATE', req, { secretId: secret._id });
    return ApiResponse.success(res, {
      secret: { _id: secret._id, name: secret.name, updatedAt: secret.updatedAt },
    }, 'Secret updated.');
  } catch (err) {
    return ApiResponse.error(res, err.message);
  }
...
exports.deleteSecret = async (req, res) => {
  try {
    ...
    emitToUser(req.userId, 'vault:deleted', { secretId: req.params.id });
